...
router.get('/contactos', async (req, res) => {
  try {
    ...
    // NUEVO: Filtro dinámico por liga
    const organizacion_id_filtro = req.query.liga_id || req.usuario.organizacion_id;
    let query = '';
    let params = [];
    ...
    if (rolNormalizado === 'superadmin') {
      query = `
        SELECT u.id, u.nombre, u.apellido, u.rol, u.organizacion_id, COALESCE(o.nombre, 'Sin Liga') as organizacion_nombre 
        FROM public.usuarios u
        LEFT JOIN public.organizaciones o ON u.organizacion_id = o.id
        WHERE u.id != $1 
        ORDER BY o.nombre, u.rol, u.nombre
      `;
      params = [id];
      ...
      params = [id, organizacion_id_filtro];
      ...
      query = `SELECT id, nombre, apellido, rol FROM public.usuarios WHERE organizacion_id = $2 AND id != $1 AND LOWER(rol) = 'administrador de liga'`;
      params = [id, organizacion_id_filtro];
    } else {
      return res.json([]);
    }
...
// 1.5 OBTENER BANDEJA DE ENTRADA
router.get('/bandeja', async (req, res) => {
  try {
    ...
    const { rows } = await db.query(`
      SELECT 
        u.id, u.nombre, u.apellido, u.rol, u.organizacion_id,
        COALESCE(o.nombre, 'Sin Liga') as organizacion_nombre,
      ...
      JOIN (
        ...
        WHERE (remitente_id = $1 OR destinatario_id = $1)
          AND organizacion_id = $2 -- FILTRO APLICADO EN LA BD
        ORDER BY 
          CASE WHEN remitente_id = $1 THEN destinatario_id ELSE remitente_id END, 
          hora_envio DESC
      ) m ON u.id = m.otro_id
